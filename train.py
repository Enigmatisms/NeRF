...
def main():
    ...
    opt_c = optim.Adam(coarse_net.parameters(), lr = 5e-4, betas = (0.9, 0.999), eps = 1e-7)
    opt_f = optim.Adam(fine_net.parameters(), lr = 5e-4, betas = (0.9, 0.999), eps = 1e-7)
    ...
    test_focal = fov2Focal(cam_fov_test, 800)
    ...
    for ep in range(epochs):
        ...
        for i in range(train_per_epoch):
            ...
            coarse_rendered, normed_weights = NeRF.render(coarse_rgbo, coarse_lengths)
            loss:torch.Tensor = loss_func(coarse_rendered, coarse_samples[:, 0, -3:])
